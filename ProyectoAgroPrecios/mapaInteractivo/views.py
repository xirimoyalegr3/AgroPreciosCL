...
def limpiar_cache_vistas(request):
    """Limpia específicamente el cache de las vistas con @cache_page"""
    try:
        from django.core.cache import cache
        from django.conf import settings
        import hashlib
        
        print(" Limpiando cache de vistas cacheadas...")
        
        # Lista de vistas que usan @cache_page
        vistas_cacheadas = [
            'dashboard_analisis',
            'obtener_resumen_general', 
            'obtener_filtros_disponibles',
            'api_grafico_precios_temporales',
            'api_grafico_distribucion_regiones',
            'obtener_datos_dashboard_reales',
            'api_grafico_estacionalidad'
        ]
        
        # Para cada vista, generar posibles claves de cache y eliminarlas
        for vista in vistas_cacheadas:
            # Las claves de cache_page incluyen la URL y parámetros
            # Generamos un patrón para buscar y eliminar
            patron_clave = f'views.decorators.cache.cache_page.{vista}'
            
            # En LocMemCache no podemos listar claves, pero podemos intentar
            # eliminar algunas combinaciones comunes
            combinaciones = [
                '',  # Sin parámetros
                'subsector=',
                'producto=',
            ...
            for combo in combinaciones:
                clave_test = f"{patron_clave}.{hashlib.md5(combo.encode()).hexdigest()}"
                cache.delete(clave_test)
        
        print(" Intentado limpiar claves de cache_page")
        
        # Método nuclear: Reiniciar todo el backend de cache
        if 'locmem' in settings.CACHES['default']['BACKEND'].lower():
            print(" Reiniciando LocMemCache completamente...")
            # Esta es la forma más efectiva con LocMemCache
            cache._cache = None
        ...
        return JsonResponse({
            'status': 'Cache de vistas limpiado (puede requerir reinicio)',
            'message': 'Es posible que necesites reiniciar el servidor para limpiar completamente el cache de @cache_page',
...
def exportar_dashboard_pdf(request):
    ...
    try:
        ...
        print(f" EXPORTANDO PDF - Filtros: subsector={subsector}, producto={producto}, año={año}, region_id={region_id}")
        
        #  USAR LA MISMA LÓGICA DEL DASHBOARD PERO SIN CACHE
        datos = obtener_datos_dashboard_filtrados(subsector, producto, año, region_id)
        
        if not datos:
            return HttpResponse("No hay datos disponibles para exportar con los filtros aplicados", status=404)
        ...
        # Título del documento con filtros
        ...
        filtros_texto = []
        if subsector: filtros_texto.append(f"Subsector: {subsector}")
        if producto: filtros_texto.append(f"Producto: {producto}")
        if año: filtros_texto.append(f"Año: {año}")
        if region_id: filtros_texto.append(f"Región ID: {region_id}")
        
        titulo = f"Reporte AgroPrecios Chile - {datetime.now().strftime('%d/%m/%Y %H:%M')}"
        if filtros_texto:
            titulo += f" | Filtros: {', '.join(filtros_texto)}"
        
        elements.append(Paragraph(titulo, title_style))
        elements.append(Spacer(1, 0.2*inch))
        
        # 1. Métricas Principales (solo si existen)
        if datos.get('metricas_principales'):
            ...
            metricas_data = [
                ['Métrica', 'Valor'],
                ['Total Registros', f"{metricas.get('total_registros_filtrados', 0):,}"],
                ['Margen Potencial', f"{metricas.get('margen_potencial_promedio', 0)}%"],
                ['Productos Alto Margen', metricas.get('productos_alto_margen', 0)],
                ['Regiones Activas', metricas.get('regiones_activas', 0)],
                ['Estacionalidad', f"{metricas.get('estacionalidad_promedio', 0)}%"]
            ]
        ...
        # 2. Oportunidades de Precios (solo primeras 5)
        if datos.get('analisis_precios') and datos['analisis_precios'].get('top_oportunidades'):
            elements.append(Paragraph("Principales Oportunidades de Precio", styles['Heading2']))
            oportunidades_data = [['Producto', 'Mejor Región', 'Precio Max', 'Precio Min', 'Margen %']]
            
            for op in datos['analisis_precios']['top_oportunidades'][:5]:
                oportunidades_data.append([
                    op.get('producto', '')[:30],  # Limitar longitud
                    op.get('mejor_region', '')[:20],
                    op.get('precio_maximo', ''),
                    op.get('precio_minimo', ''),
                    f"{op.get('margen_potencial', 0)}%"
                ])
            
            op_table = Table(oportunidades_data, colWidths=[1.5*inch, 1.2*inch, 1*inch, 1*inch, 0.8*inch])
        ...
        nombre_archivo = f"reporte_agroprecios_{datetime.now().strftime('%Y%m%d_%H%M')}"
        if region_id:
            nombre_archivo += f"_region_{region_id}"
        if producto:
            nombre_archivo += f"_{producto[:10]}"
        nombre_archivo += ".pdf"
        
        response = HttpResponse(buffer, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="{nombre_archivo}"'
        
        print(f" PDF exportado exitosamente: {nombre_archivo}")
        return response
...
def exportar_dashboard_excel(request):
    ...
    try:
        ...
        print(f" EXPORTANDO EXCEL - Filtros: subsector={subsector}, producto={producto}, año={año}, region_id={region_id}")
        
        #  USAR LA MISMA LÓGICA DEL DASHBOARD
        datos = obtener_datos_dashboard_filtrados(subsector, producto, año, region_id)
        ...
        # Estilos básicos (sin muchos formatos para mayor velocidad)
        ...
        ws_resumen['A1'] = "REPORTE AGROPRECIOS CHILE"
        ws_resumen['A1'].font = Font(bold=True, size=12, color="2c5aa0")
        ws_resumen.merge_cells('A1:E1')
        
        ws_resumen['A2'] = f"Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M')}"
        ws_resumen['A2'].font = Font(italic=True, size=9, color="666666")
        ws_resumen.merge_cells('A2:E2')
        
        # Filtros aplicados
        fila_actual = 4
        if subsector or producto or año or region_id:
            ws_resumen[f'A{fila_actual}'] = "FILTROS APLICADOS:"
            ws_resumen[f'A{fila_actual}'].font = Font(bold=True, size=10)
            fila_actual += 1
            
            if subsector:
                ws_resumen[f'A{fila_actual}'] = f"Subsector: {subsector}"
                fila_actual += 1
            if producto:
                ws_resumen[f'A{fila_actual}'] = f"Producto: {producto}"
                fila_actual += 1
            if año:
                ws_resumen[f'A{fila_actual}'] = f"Año: {año}"
                fila_actual += 1
            if region_id:
                ws_resumen[f'A{fila_actual}'] = f"Región ID: {region_id}"
                fila_actual += 1
        ...
        if datos.get('metricas_principales'):
            metricas = datos['metricas_principales']
            ws_resumen[f'A{fila_actual}'] = "MÉTRICAS PRINCIPALES"
            ws_resumen[f'A{fila_actual}'].font = Font(bold=True, size=11)
            fila_actual += 1
            
            metricas_data = [
                ['Total Registros', metricas.get('total_registros_filtrados', 0)],
                ['Margen Potencial Promedio', f"{metricas.get('margen_potencial_promedio', 0)}%"],
                ['Productos con Alto Margen', metricas.get('productos_alto_margen', 0)],
                ['Regiones Activas', metricas.get('regiones_activas', 0)],
                ['Estacionalidad Promedio', f"{metricas.get('estacionalidad_promedio', 0)}%"]
            ]
            
            for i, (label, valor) in enumerate(metricas_data):
                ws_resumen[f'A{fila_actual + i}'] = label
                ws_resumen[f'B{fila_actual + i}'] = valor
            
            fila_actual += len(metricas_data) + 2
        
        # 2. HOJA OPORTUNIDADES
        if datos.get('analisis_precios') and datos['analisis_precios'].get('top_oportunidades'):
            ws_oportunidades = wb.create_sheet("Oportunidades de Precio")
            
            headers = ['Producto', 'Mejor Región', 'Precio Máximo', 'Precio Mínimo', 'Diferencial', 'Margen %']
            for col, header in enumerate(headers, 1):
                cell = ws_oportunidades.cell(row=1, column=col, value=header)
                cell.font = header_font
                cell.fill = header_fill_green
                cell.alignment = center_align
            
            for row_idx, op in enumerate(datos['analisis_precios']['top_oportunidades'][:20], 2):  # Máximo 20
                ws_oportunidades.cell(row=row_idx, column=1, value=op.get('producto', ''))
                ws_oportunidades.cell(row=row_idx, column=2, value=op.get('mejor_region', ''))
                ws_oportunidades.cell(row=row_idx, column=3, value=op.get('precio_maximo', ''))
                ws_oportunidades.cell(row=row_idx, column=4, value=op.get('precio_minimo', ''))
                ws_oportunidades.cell(row=row_idx, column=5, value=op.get('diferencial_precio', ''))
                ws_oportunidades.cell(row=row_idx, column=6, value=op.get('margen_potencial', ''))
        
        # 3. HOJA VOLÚMENES
        if datos.get('analisis_volumenes') and datos['analisis_volumenes'].get('top_regiones_volumen'):
            ws_volumenes = wb.create_sheet("Análisis de Volúmenes")
            
            headers = ['Región', 'Producto Principal', 'Volumen Total', 'Mercados Activos', 'Liquidez']
            for col, header in enumerate(headers, 1):
                cell = ws_volumenes.cell(row=1, column=col, value=header)
                cell.font = header_font
                cell.fill = header_fill_blue
                cell.alignment = center_align
            
            for row_idx, vol in enumerate(datos['analisis_volumenes']['top_regiones_volumen'][:15], 2):  # Máximo 15
                ws_volumenes.cell(row=row_idx, column=1, value=vol.get('region', ''))
                ws_volumenes.cell(row=row_idx, column=2, value=vol.get('producto_mas_transado', ''))
                ws_volumenes.cell(row=row_idx, column=3, value=vol.get('volumen_total', ''))
                ws_volumenes.cell(row=row_idx, column=4, value=vol.get('mercados_activos', ''))
                ws_volumenes.cell(row=row_idx, column=5, value=vol.get('liquidez', ''))
...
def obtener_datos_dashboard_filtrados(subsector, producto, año, region_id):
    """Función optimizada para obtener datos del dashboard con filtros"""
    try:
        print(f" OBTENIENDO DATOS FILTRADOS - subsector={subsector}, producto={producto}, año={año}, region_id={region_id}")
        
        #  CONSULTA DIRECTA SIN USAR RequestFactory
        queryset = DatosComercializacion.objects.select_related(
        ...
        # Métricas básicas para exportación
        ...
        oportunidades = (queryset
            .values('producto__nombre', 'region__nombre')
            .annotate(
                precio_max=Max('precio_promedio'),
                precio_min=Min('precio_promedio'),
                total_registros=Count('id')
            ...
            .filter(margen__gt=10, margen__lt=300)
            .order_by('-margen')[:10])
        ...
        datos = {
            'metricas_principales': {
                'total_registros_filtrados': metricas['total_registros'] or 0,
                'margen_potencial_promedio': 25.0,  # Placeholder simple
                'productos_alto_margen': len([op for op in oportunidades if op['margen'] > 50]),
                'regiones_activas': volumenes.count(),
                'estacionalidad_promedio': 15.0
            },
            'analisis_precios': {
                'top_oportunidades': [
                    {
                        'producto': op['producto__nombre'],
                        'mejor_region': op['region__nombre'],
                        'peor_region': 'Otras regiones',
                        'precio_maximo': f"${op['precio_max']:.0f}",
                        'precio_minimo': f"${op['precio_min']:.0f}",
                        'diferencial_precio': f"${op['precio_max'] - op['precio_min']:.0f}",
                        'margen_potencial': round(op['margen'], 1)
            ...
            'analisis_volumenes': {
                'top_regiones_volumen': [
                    {
                        'region': vol['region__nombre'],
                        'producto_mas_transado': 'Varios productos',
                        'volumen_total': f"{vol['volumen_total']:,.0f}",
                        'mercados_activos': vol['mercados_activos'],
                        'liquidez': round(vol['volumen_total'] / 100000, 1) if vol['volumen_total'] else 0
                    } for vol in volumenes
                ]
            }
        }
        
        print(f" DATOS OBTENIDOS - {len(oportunidades)} oportunidades, {len(volumenes)} regiones")
        return datos
        
    except Exception as e:
        print(f" Error en obtener_datos_dashboard_filtrados: {e}")
        # Datos de ejemplo como fallback
        return {
            'metricas_principales': {
                'total_registros_filtrados': 1000,
                'margen_potencial_promedio': 30.5,
                'productos_alto_margen': 8,
                'regiones_activas': 5,
                'estacionalidad_promedio': 25.0
            },
            'analisis_precios': {
                'top_oportunidades': [
                    {
                        ...
                        'peor_region': 'Otra región',
                        'precio_maximo': '$1,500',
                        'precio_minimo': '$1,000',
                        'diferencial_precio': '$500',
            ...
            'analisis_volumenes': {
                'top_regiones_volumen': [
                    {
                        'region': 'RM Santiago',
                        'producto_mas_transado': 'Producto Ejemplo',
                        'volumen_total': '100,000',
                        'mercados_activos': 3,
                        'liquidez': 8.5
                    }
                ]
